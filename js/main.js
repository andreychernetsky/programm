...
const getFunAcc = ()=>{
	...
	accButton.forEach(btn=>btn.addEventListener('click', ()=>{
			const openAcc = btn.nextElementSibling.classList.contains('accordion__panel--open');
			if(openAcc) {
					closeAccFun(btn.nextElementSibling);
			}else {
					accButton.forEach(x=>closeAccFun(x.nextElementSibling));
					openAccFun(btn.nextElementSibling);
			}
	}));
	const openAccFun= (item)=>{
			item.classList.add('accordion__panel--open')
			item.style.maxHeight = item.scrollHeight + 'px';
			// item.style.padding = ''
	}
	const closeAccFun = (item)=>{
